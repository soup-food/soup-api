...
class FoodBase(BaseModel):
    ...
    name: str = Field(
        ..., min_length=2, max_length=100, description="Name of the food item."
    )
    calories: int = Field(..., gt=0, description="Caloric content per serving.")
    description: str = Field(
        ..., max_length=500, description="A brief description of the food item."
    )
...
class Food(FoodBase):
    ...
    )  # Allows initialization with `_id` or `id`
...
class FoodUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=2, max_length=100, description="Updated name of the food item."
    )
    calories: Optional[int] = Field(
        None, gt=0, description="Updated caloric content per serving."
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Updated description of the food item."
    )
